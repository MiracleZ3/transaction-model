...
当 `make docker-build-*` 失败、或服务器不便运行 make 时，可直接调 `docker build`。
镜像之间存在依赖（tm-cpu 继承 tm-base；GPU 镜像另起），构建顺序见下文。

#### 镜像清单与文件

| 镜像 tag | Dockerfile | 用途 | 大小 |
|----------|-----------|------|------|
| `local/tm-base:latest` | `docker/Dockerfile.base` | CPU base（不直接运行，给 tm-cpu 当 FROM） | ~600 MB |
| `local/tm-cpu:latest`  | `docker/Dockerfile.cpu`  | Step 1/5 + pytest + 可视化（CPU 即可） | ~1.2 GB |
| `local/tm-gpu-train:latest` | `docker/Dockerfile.gpu.train` | Step 2/3（CUDA + cuDF + NeMo） | ~10 GB |
| `local/tm-gpu-infer:latest` | `docker/Dockerfile.gpu.infer` | Step 4（CUDA + cuDF + transformers） | ~8 GB |

四个 `Dockerfile.*` 都用 BuildKit 的 `--mount=type=cache` 加速 apt/pip 层。
正常情况下 `make` 会自动用 BuildKit，可工作；但如果 daemon 状态异常
（典型错误：`error running prestart hook ... dial unix /var/run/docker/libnetwork/*.sock`），
请用下面的 **`DOCKER_BUILDKIT=0`** 退化方案——它绕开 BuildKit 子进程，
不再依赖 libnetwork socket。

#### 仅 CPU（推荐作为冒烟/CI 起步）

```bash
# 1. base（BuildKit 正常时；与 `make docker-build-base` 等价）
...
```bash
# 同上的 base + cpu 后，再构建：
docker build -f docker/Dockerfile.gpu.train -t local/tm-gpu-train:latest .
docker build -f docker/Dockerfile.gpu.infer -t local/tm-gpu-infer:latest .
docker images | grep '^local/tm-'   # 列出四张镜像确认
```

#### 故障回退：禁用 BuildKit（解决 `libnetwork / prestart hook` 错误）

_servers 报这类错时直接这么做，不必重启 Docker daemon：_

```bash
# Step 1: 在 /tmp 下生成"去掉 --mount=type=cache"的临时副本
#         (classic builder 不识别 cache mount 语法，必须先剥掉)
mkdir -p /tmp/dl-tmpl && python3 - <<'PY'
import re, pathlib
src = pathlib.Path("docker")
dst = pathlib.Path("/tmp/dl-tmpl"); dst.mkdir(exist_ok=True)
for name in ("Dockerfile.base","Dockerfile.cpu","Dockerfile.gpu.train","Dockerfile.gpu.infer"):
    lines = (src/name).read_text().splitlines(keepends=True)
    kept = []
    for ln in lines:
        # 'RUN --mount=... \\\n'  →  'RUN \\\n'   (保留 RUN，让下一行接得住)
        if re.match(r'^RUN --mount=type=cache.*\\\n

#### 镜像构建后怎么用

`Dockerfile.cpu` 与 `Dockerfile.gpu.train` 都把 `docker/entrypoint.sh` 作为
...
docker run --rm -v $(pwd)/data:/workspace/data \
    -v $(pwd)/models:/workspace/models \
    --gpus all local/tm-gpu-train:latest train --demo    # Step 3
...
> Route A（银联 NDJSON）：把对应 entrypoint subcommand 换成
> `python scripts/step_01b_load_ndjson.py ...` 这类直接调用，因为
> YL/Route C 脚本尚未加入 entrypoint subcommand 列表。
...
, ln):
            ...
            continue
        # '    --mount=... \\\n'  →  整行删（仅是续行）
        if re.match(r'^\s+--mount=type=cache.*\\\n
...
, ln):
            continue
        kept.append(ln)
    (dst/name).write_text("".join(kept))
print("Wrote 4 Dockerfiles → /tmp/dl-tmpl/")
PY

# Step 2: 用 classic builder build 四张镜像
DOCKER_BUILDKIT=0 docker build -f /tmp/dl-tmpl/Dockerfile.base \
    -t local/tm-base:latest .
DOCKER_BUILDKIT=0 docker build -f /tmp/dl-tmpl/Dockerfile.cpu \
    --build-arg REGISTRY=local --build-arg TAG=latest \
    -t local/tm-cpu:latest .
DOCKER_BUILDKIT=0 docker build -f /tmp/dl-tmpl/Dockerfile.gpu.train \
    -t local/tm-gpu-train:latest .
DOCKER_BUILDKIT=0 docker build -f /tmp/dl-tmpl/Dockerfile.gpu.infer \
    -t local/tm-gpu-infer:latest .

# Step 3: 清理临时副本
rm -rf /tmp/dl-tmpl
```

输出与 BuildKit 模式一致；唯一的区别是缺少 apt/pip cache 跨 build 复用，
所以连续 build 同一镜像会稍慢（首层重跑大约 30-90 秒）。
