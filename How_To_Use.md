...
- ❌ DeepSpeed / FSDP 暂未启用 Route C 下游（trainer 走 torch 原生 DDP）。
...
```bash
# 检查 world_size 是否对：跑 demo、看 stdout
NGPUS=2 bash scripts/routec_ddp_single_node.sh --demo

# 应看到类似：
# Route C 微调：task_type=lora loss=sft_focal_loss_with_amount
#   rank/world_size: 0/2 (local_rank=0)
#   rank/world_size: 1/2 (local_rank=1)
```

如果是 `[rank 0] ... DDP wrapped: rank=0/2 local_rank=0`，DDP 起来了。
