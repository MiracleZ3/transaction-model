...
# 1. 单元 + 集成测试（CPU）
pytest tests/ -v              # ~25 个测试，~6 秒
# auto-fallback to examples/sample_data/smoke.jsonl

# 2. Route A 路线：NDJSON → 行级 parquet
python scripts/step_01b_load_ndjson.py \
    --ndjson-dir examples/sample_data \
    --no-gpu                   # 当前目录读到 data/yl/temporal_split/{train,val,test}.parquet

# 3. Route A 路线：行级 parquet → tokenized corpus
#    需 GPU + cuDF（CPU 跑不了 step_02）。这一步在 CI 上 skip。

# 4. Route C 模型层冒烟（CPU 即可，不读 sample）
