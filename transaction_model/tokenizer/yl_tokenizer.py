...
class YLTabularTokenizer:
    ...
    def _get_pipeline_state(self) -> dict:
        """序列化每个 step 的 fitted vocab（用于 from_file 重建 transform 能力）。"""
        steps_state = {}
        for tok_id in self._pipeline.tokenizer_order:
            tok = self._pipeline.steps[tok_id]
            # NC = NumericalTokenizerOptBin 含 builder 不能直接 json 化；
            # transform 用 encode 只依赖 vocab 字符串映射，这里只存 idx_to_token。
            idx_to_tok = tok._idx_to_token
            if isinstance(idx_to_tok, dict):
                # 转成 str-int key（NC 的 idx_to_tok 已是 {int:str}）
                serialized = {
                    "idx_to_token": {
